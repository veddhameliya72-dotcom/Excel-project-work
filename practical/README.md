...
- [📈 Sheet 6 — Dashboard Resources](#-sheet-6--dashboard-resources)
...
└── 📁 assets/
    ...
    └── 🖼️ 08_dashboard_resources.png          ← 6 helper tables behind the dashboard (real screenshot)
```

> **On the screenshots:** every image in `assets/` is a genuine screenshot of the workbook itself (rendered from the real `.xlsx`, not redrawn) — same fonts, gridlines, colors, and numbers you'd see opening the file. One caveat: three chart *objects* (the Pivots sheet's monthly-trend line, the Regression scatter-with-trendline, and the What-If discount bar chart) reference very large ranges that the automated screenshot tool couldn't rasterize, so those specific chart pictures are cropped out of the images below — but every underlying data table is shown in full, and the identical monthly-trend chart renders correctly on the Dashboard screenshot.
...
## 📈 Sheet 6 — Dashboard Resources

A hidden-in-plain-sight helper sheet: 6 small `SUMIFS`-built tables that feed every chart, KPI card, and filter box on the Dashboard — nothing on the Dashboard references `Cleaned Data` directly, which keeps the dashboard itself fast and simple.

![Dashboard Resources](assets/08_dashboard_resources.png)

| Helper Table | Feeds |
|---|---|
| 1. Sales Trend by Month (48 rows) | The monthly trend line chart |
| 2. Profit by Sub-Category (ascending) | The sub-category bar chart |
| 3. Sales by Segment | (cross-check table for the segment donut) |
| 4. Region List | The Region filter box |
| 5. Category List | The Category filter box |
| 6. Order Date Range | The Timeline slicer's bounds (02-Jan-2014 to 30-Dec-2017) |
